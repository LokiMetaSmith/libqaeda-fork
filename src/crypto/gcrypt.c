...
struct gpg_store {
	...
	char private_key[LQ_PRIVKEY_LEN]; ///< Literal, uncompressed private key bytes. Used in LQPrivKey.
...
static int key_apply_private(struct gpg_store *gpg) {
	char *p;
	size_t c;
	gcry_sexp_t one;
	gcry_sexp_t two;

	one = gcry_sexp_find_token(gpg->k, "private-key", 11);
	if (one == NULL) {
		return debug_logerr(LLOG_ERROR, ERR_KEYFAIL, "sexp privkey");
	}
	two = gcry_sexp_find_token(one, "d", 1);
	if (two == NULL) {
		gcry_sexp_release(one);
		return debug_logerr(LLOG_ERROR, ERR_KEYFAIL, "sexp d");
	}
	c = LQ_PRIVKEY_LEN;
	...
	if (p == NULL) {
		...
		gcry_sexp_release(one);
		return debug_logerr(LLOG_ERROR, ERR_KEYFAIL, "sexp d data");
	}

	lq_cpy(gpg->private_key, p, LQ_PRIVKEY_LEN);
	gcry_sexp_release(two);
	gcry_sexp_release(one);
...
size_t lq_privatekey_bytes(LQPrivKey *pk, char **out) {
	...
	if (lq_cmp(gpg->private_key, zeros, LQ_PRIVKEY_LEN)) {
		r = key_apply_private(gpg);
		if (r != ERR_OK) {
			return 0;
		}
	}
	*out = gpg->private_key;
	return LQ_PRIVKEY_LEN;
}
